...
# ╔═╡ f4b1cbb2-dce0-11ee-15b3-2b434f7f8490
using VLBIData

# ╔═╡ 2bbbc616-e51b-411d-9228-b552f172749e
using InterferometricModels

# ╔═╡ d05874b1-9d57-42d8-94af-e26b006aa41e
using VLBIPlots

# ╔═╡ 1445f8a2-20c0-4b1c-affc-c7cace63e292
using StaticArrays

# ╔═╡ a7bb9080-001c-4433-b140-6fda88b5d1e0
using CairoMakie

# ╔═╡ 9cabef90-3944-4937-8fe6-27baabdbf776
using MakieExtra

# ╔═╡ 8761d629-0737-4ef4-aa4e-434e2355334c
using DataManipulation

# ╔═╡ 3e131fa1-802f-495a-8556-2f699ee7f4e9
using Unitful, UnitfulAngles

# ╔═╡ 47bdcb76-b50c-4394-b231-70a3dbcbd6b2
using IntervarrrSets

# ╔═╡ 377a1c37-43a6-421a-9445-c46eeb688661
using Colors

# ╔═╡ a6d09019-02b7-43b1-a06b-b763626876e6
using DirectionalStatistics

# ╔═╡ 544500f7-8fbd-47fc-9965-ee90ec89f02e
using RectiGrids

# ╔═╡ 653c39b0-2fd9-425e-b428-9e02a6dab35a
using AxisKeysExtra

# ╔═╡ 9f39e74c-02cf-42cb-b311-578dfb7b99a0
using Statistics: mean

# ╔═╡ bed0f6d4-644d-4a8b-8146-de35467a0c87
using LinearAlgebra

# ╔═╡ 535c3da9-83e5-48e2-af6e-d467205ff8b4
import GLMakie

# ╔═╡ e6dbeda0-a65e-4ced-a067-b7d423dcc8e0
add_conj_vis(uvtbl) = @p let
	uvtbl
	__[sr"uv$", sr"visibility", sr"weight"]
	orig = __
...
# ╔═╡ e6cab34b-b4d5-4308-9ed7-5f0a57d71ed3
uvtbl = table(VLBI.load(joinpath(pkgdir(VLBI), "/Users/aplavin/work/rfc/images/J1256-0547/J1256-0547_U_2010_06_27_moj_vis.fits"))) |> filter(x -> x.stokes ∈ (:RR, :LL)) |> add_conj_vis

# ╔═╡ 972384ba-20f6-42cb-8a2b-04f4830e1388
let
	fig, ax1, plt = scatter(RadPlot(uvtbl), markersize=1)
	ax2, plt = scatter(fig[2,1], RadPlot(uvtbl, yfunc=rad2deg∘angle), markersize=1) #, axis=(yticks=Makie.AngularTicks(rad2deg(1), "°"),))

	model = EllipticGaussian(flux=7, σ_major=1u"mas", ratio_minor_major=0.3, pa_major=0.5, coords=SVector(0.1, 0.5)u"mas")
	scatter!(ax1, RadPlot(uvtbl; model), markersize=2, color=:red)
	scatter!(ax2, RadPlot(uvtbl; model, yfunc=rad2deg∘angle), markersize=2, color=:red)
	
	ax1.xlabelvisible = false
	ax1.xticklabelsvisible = false
	linkxaxes!(ax1, ax2)
	fig
	# display(fig, backend=CairoMakie)
end

# ╔═╡ 52b61b29-ac7f-41fa-b0e2-fe7534d4ea9d
let
	model = EllipticGaussian(flux=1, σ_major=1u"mas", ratio_minor_major=0.3, pa_major=0.5, coords=SVector(0.1, 0.5)u"mas")
	fig, ax1, plt = band(RadPlot(0..2e8; model))
	ax2, plt = band(fig[2,1], RadPlot(0..2e8; model, yfunc=rad2deg∘angle))
	ax1.xlabelvisible = false
	ax1.xticklabelsvisible = false
	linkxaxes!(ax1, ax2)
	fig
...
# ╔═╡ 53909945-926c-48e7-9e0d-10c7e0952c5b
let
	fig, ax1, plt = scatter(ProjPlot(uvtbl, 0), markersize=1)
	ax2, plt = scatter(fig[2,1], ProjPlot(uvtbl, 0, yfunc=rad2deg∘angle), markersize=1)

	model = EllipticGaussian(flux=7, σ_major=1u"mas", ratio_minor_major=0.3, pa_major=0.5, coords=SVector(0.1, 0.5)u"mas")
	scatter!(ax1, ProjPlot(uvtbl, 0; model), markersize=2, color=:red)
	scatter!(ax2, ProjPlot(uvtbl, 0; model, yfunc=rad2deg∘angle), markersize=2, color=:red)
...
# ╔═╡ 5e487757-f9be-42cd-918a-4a35b59c500e
let
	fig, ax1, plt = scatter(UVPlot(uvtbl), markersize=1, color=(@p uvtbl map(atan(_.uv...))), colormap=:cyclic_mrybm_35_75_c68_n256)
	fig
end

# ╔═╡ e5f3803d-ffeb-438e-be32-dc440cdf5e46
# let
# 	fig, ax1, plt = scatter(UVPlot(uvtbl), markersize=1, color=x -> atan(x.uv...))
# 	fig
# end

# ╔═╡ f4156676-c05a-4627-a13b-d36a57b2d2e9
# let
	posang = Observable(0.)

	Δang = Observable(30u"°")
	# fig = Figure(size=(500, 300)) #, fontsize=7)
	fig = Figure(size=(1500, 500)) #, fontsize=7)

	colormap = Makie.ColorSchemes.cyclic_mrybm_35_75_c68_n256_s25
	colors = (@lift @p uvtbl map() do __
		mod(atan(__.uv...), 0..π)
		coloralpha(get(colormap, __/π), Circular.distance($posang, __, period=π) < $Δang ? 1 : 0.015)
	end)
	
	ax1, plt = scatter(fig[1,-1], (RadPlot(uvtbl)), color=colors, colormap=:cyclic_mrybm_35_75_c68_n256, markersize=1, marker=Makie.FastPixel())
	ax2, plt = scatter(fig[2,-1], (RadPlot(uvtbl, yfunc=rad2deg∘angle)), color=colors, colormap=:cyclic_mrybm_35_75_c68_n256, markersize=1, marker=Makie.FastPixel(), axis=(xtickformat=EngTicks(:symbol),))
	ax1.xlabelvisible = false
	ax1.xticklabelsvisible = false
	linkxaxes!(ax1, ax2)

	ax0, plt = scatter(fig[1:2,0], UVPlot(uvtbl), markersize=1, color=colors, colormap=:cyclic_mrybm_35_75_c68_n256, axis=(xtickformat=EngTicks(:symbol), ytickformat=EngTicks(:symbol),))
	lines!(ax0, (@lift @p SVector(sincos($posang)) [__, -__] .+ Ref(SVector(0.5, 0.5))), color=:black, space=:relative)
	
	ax1, plt = scatter(fig[1,1], (@lift ProjPlot(uvtbl, $posang)), color=colors, colormap=:cyclic_mrybm_35_75_c68_n256, markersize=1, marker=Makie.FastPixel())
	ax2, plt = scatter(fig[2,1], (@lift ProjPlot(uvtbl, $posang, yfunc=rad2deg∘angle)), color=colors, colormap=:cyclic_mrybm_35_75_c68_n256, markersize=1, marker=Makie.FastPixel(), axis=(xtickformat=EngTicks(:symbol),))
	ax1.xlabelvisible = false
	ax1.xticklabelsvisible = false
	linkxaxes!(ax1, ax2)
	...
	Record(fig, range(0, 2π, length=20), framerate=20) do posang_
		posang[] = posang_
	end
end

# ╔═╡ 2270c553-b122-405b-baf4-129c36c2965c


# ╔═╡ ce7d7607-8075-450e-8d69-de91f7125c3e


# ╔═╡ c83d2022-17b6-46d4-9dbd-0b91c0670fc2
scatter(maprange(log, 1e-10, 1e10, length=100), axis=(yscale=log10, ytickformat=EngTicks(:symbol)))

# ╔═╡ 976e13c6-449f-4802-9063-6af1cd05d048


# ╔═╡ 6503466a-49c5-4cb8-916c-66f3e098850b
Accessors.set(m::Makie.GeometryBasics.Mesh, ::PropertyLens{:position}, position) = 
	Makie.GeometryBasics.Mesh(position, Makie.GeometryBasics.faces(m))
...
IntervarrrSets = "a884e8af-a1e7-4ed8-8a37-289263ed986d"
...
DirectionalStatistics = "~0.1.25"
GLMakie = "~0.9.10"
InterferometricModels = "~0.1.12"
IntervarrrSets = "~0.1.2"
MakieExtra = "~0.1.2"
RectiGrids = "~0.1.18"
StaticArrays = "~1.9.3"
Unitful = "~1.19.0"
...
manifest_format = "2.0"
project_hash = "1951004cc255839220678b9b27a5222c25f801dd"
...
git-tree-sha1 = "7e413f81cee8182edc5ed55fc760a2c0568736d6"
...
[[deps.DataPipes]]
git-tree-sha1 = "1e460b665b1822f751da3509a3a4aa5b467b44a1"
...
git-tree-sha1 = "93af4608d8c9cf2a4a2d0885125bd49dccdb509d"
uuid = "e814f24e-44b0-11e9-2fd5-aba2b6113d95"
version = "0.1.25"
weakdeps = ["Distributions"]

    [deps.DirectionalStatistics.extensions]
    DistributionsExt = "Distributions"
...
git-tree-sha1 = "a1e9dd730bfa9dff8804ec3a14575531e0d44558"
...
version = "0.1.25"
weakdeps = ["AxisKeys", "Dictionaries", "IntervalSets", "StructArrays"]
...
git-tree-sha1 = "b811128a9f605f12607d0a8e6bf73d501cd8a842"
...
[[deps.IntervarrrSets]]
deps = ["Accessors", "IntervalSets", "Reexport"]
git-tree-sha1 = "dd4c4e3b71e736ecf84b7dd7903439f48283b4a9"
uuid = "a884e8af-a1e7-4ed8-8a37-289263ed986d"
version = "0.1.2"
weakdeps = ["Unitful"]

    [deps.IntervarrrSets.extensions]
    UnitfulExt = "Unitful"
...
git-tree-sha1 = "4f2b7fdc34b03f35b94b276c6f57fb9623fb0171"
...
[[deps.Skipper]]
git-tree-sha1 = "e347dafb2f17473987ef6bc0c8e59a4bb4dd3750"
...
[[deps.VLBIPlots]]
deps = ["Accessors", "DataPipes", "InterferometricModels", "IntervalSets", "LinearAlgebra", "MakieExtra", "Unitful"]
git-tree-sha1 = "68cfb942313ef00b3fc763e60c3cf8aa6d7873ed"
